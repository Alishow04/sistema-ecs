...
// dá pra conferir os últimos criados sem trocar de tela enquanto se cria
// um novo. Por isso "Últimos ECS" deixou de ser um item separado do menu.
...
async function renderFormCard(formColumn, user, miniListRegion) {
  ...
  formColumn.querySelector('[data-cancel]').addEventListener('click', () => form.reset());
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      refreshMiniList(miniListRegion); // o ECS recém-criado já aparece no painel ao lado
    } catch (err) {
      console.error(err);
