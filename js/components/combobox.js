...
export function createCombobox({
  ...
  container.innerHTML = `
    <input type="text" autocomplete="off" placeholder="${placeholder}" value="${initialValue}" />
  ...
  function renderList(filterText) {
    const filtered = options.filter((o) => normalize(o).includes(normalize(filterText)));
    highlighted = -1;
  ...
  input.addEventListener('blur', () => {
    setTimeout(() => {
      ...
      if (!allowFreeText && input.value !== currentValue) {
        // valor digitado não confere com uma opção válida -> reverte
